...
module PokerScorer =
    ...
    let toSingles = function
    | [c1;c2;c3;c4;c5] -> (c1,c2,c3,c4,c5)
    let createFlush cards =
        cards |> List.sortDescending |> toSingles |> Flush

    let cardsFromCardGroups (groups:GroupedCards) =
        groups |> List.collect snd
    let allGroupsSameSuit = cardsFromCardGroups >> sameSuit
    ...
    | cardGroups when allGroupsSameSuit cardGroups -> 
        cardGroups |> (cardsFromCardGroups >> createFlush)
    | [((3,_),[c1;c2;c3]);(_,[k1]);(_,[k2])] -> ThreeOfaKind ((c1,c2,c3),(k1,k2))
    | [((2,_),[c1;c2]);((2,_),[c3;c4]);(_,[k1])] -> DoublePair ((c1,c2),(c3,c4),k1)
    | [((2,_),[c1;c2]);(_,[k1]);(_,[k2]);(_,[k3])] -> OnePair ((c1,c2),(k1,k2,k3))
